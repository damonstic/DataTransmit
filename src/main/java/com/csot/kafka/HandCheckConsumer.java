...
public class HandCheckConsumer {
    ...
    @Value("${json.file.path_hand01}")
    private String jsonFilePath01;
    @Value("${json.file.path_hand02}")
    private String jsonFilePath02;

    @Autowired
    private HttpPostSenderHand httpPostSender;
    // 记录当前应该存储到哪个文件的标记
    private boolean writeToFirstFile = true;
    @KafkaListener(topics = {"handcheck"},groupId = "cassandranew")
    public void listen(List<String> hands) throws MessagingException, ParseException {
        ...
        saveDataToJson(hands, jsonFilePath);
